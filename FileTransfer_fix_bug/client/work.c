...
//	printf("head_len = %d ; send head: sendsize = %d\n",head_len, sendsize);
    ...
    /************************************ 新增修复内容 *************************************/
    int last_little_size = p_fhead->bs % SEND_SIZE;
    if (last_little_size > 0) {
        if( (send_size = send(sock_fd, fp, last_little_size, 0)) == last_little_size){
            fp += last_little_size;
            printf("fp = %p ; a last_little_size ok\n", fp);
        } else {
            printf("send_size = %d ;  a last_little_size erro\n",send_size);
        }
    } else
        printf("send_size = %d ;  a SEND_SIZE erro\n",send_size);
